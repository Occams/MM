...
\begin{frame}
	\begin{itemize}
		...
		\item Die Berechnung der DCT-Koeffizienten für jeden Block und das anschließende Sortieren tragen den größten Teil zur Laufzeit bei.
		\item Bei allen Ansätzen wurde die DCT-Berechnung parallelisiert.
    \end{itemize}
\end{frame}
...
\begin{frame}
	\begin{itemize}
		...
		\item Es werden genau $256 * 2 + 1$ FLOPS für ein Element der DCT-Koeffizientenmatrix benötigt.
		\item Die zu bearbeitenden Blöcke werden unter den Threads aufgeteilt.
    \end{itemize}
\end{frame}
...
\begin{frame}
	\begin{itemize}
		...
		\item	\begin{equation*}
					A * M * A = DCT
				\end{equation*}
		\item Insgesamt werden nur $64*2 + 1$ FLOPS für ein Element der DCT-Koeffizientenmatrix benötigt.
		\item Die zu bearbeitenden Blöcke werden unter den Threads aufgeteilt.
    \end{itemize}
\end{frame}
...
\begin{frame}
	\begin{itemize}
		\item Idee: Die \emph{relevanten} Elemente der DCT-Koeffizientenmatrix stehen \enquote{links oben}. Hier werden sich also die Blöcke am wahrscheinlichsten unterscheiden.
		\item Berechnung der ersten $16$ Werte der DCT-Matrix jedes Blockes (Eine $4x4$ Matrix \enquote{links oben}).
		\item Sortierung der Blöcke und anschließendes Aussortieren von Blöcken die keine Duplikate besitzen.
		\item Ermittlung der restlichen Elemente der Koeffizientenmatrizen für die Menge von Blöcken die mindestens einen Partner besitzen.
		\item Diese ist je nach Bilddatei ca. halb so groß wie die ursprüngliche Menge an Blöcken.
		\item Die zu bearbeitenden Blöcke werden unter den Threads aufgeteilt.
    \end{itemize}
\end{frame}
...
\begin{frame}
	Funktionalität:
	\begin{itemize}
		\item Bilddatei laden/öffnen, Auswahl einer Implementierung des Algorithmus, Starten/Abbruch des ausgewählten Algorithmus
		...
		\item Die ermittelten Shiftvektoren können anhand ihrer Länge dynamisch gefiltert werden.
    \end{itemize}
\end{frame}
